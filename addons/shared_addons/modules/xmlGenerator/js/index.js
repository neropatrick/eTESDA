(function($) {
	$(function() {
		...
		$item_list.find('li a').live('click', function(e) {
			...
			$details.load(SITE_URL+'admin/syllabus/load_syllabus_details/'+syllabus_id, function(){
		...
		post_sort_callback = function() {
			...
			if($details_id.val() > 0)
			{
				$details.load(SITE_URL + $details_id.val());
