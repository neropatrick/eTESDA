...
//                      ->build('test_onlineAssessment');
    ...
    public function previewOA($courseid){
        $questions = array();
            $answers = array();
            $correct = array();
            $sectionctr = 1;
...
//        }
        ...
        if($this->form_validation->run()){
            $_SESSION['postans'] = $_POST;
//            print_r($_SESSION['postans']);
//            print_r($this->input->post('ans1'));
...
//        
       ...
       // print_r($answers);
        ...
        foreach ($postans as $key=>$postitems) {
        ...
        $badDets = $this->model_users->badSCName($thisCourse, $sections->moduleID);
        $badgeImg = $this->model_users->badSCImg($thisCourse, $sections->moduleID);
...
//        }
    ...
    public function previewSelfCheck(){
        ...
        for($i = 1; $i<=count($correct[1]); $i++){
         ...
         if($this->form_validation->run()){
            $_SESSION['postans'] = $_POST;
            redirect('test/previewSCResults');
        }
        else{
    ...
    public function previewSC($moduleid){
        unset($_SESSION['answers']);
        unset($_SESSION['sections']);
        unset($_SESSION['questions']);
        //echo $moduleid;
...
//              print_r((object)$answers);
        ...
        $_SESSION['correct'] = $correct1;
         ...
         redirect('test/previewSelfCheck');
        }
        
        public function previewSCResults() {
...
//        }
    ...
    public function shareSCFB($resID){
        ...
        redirect('syllabus/dispMods2/'.$forShare[0]['id']);
